...
class ImageUploadForm extends Model
{
    ...
    public function sendImages($images,$cid)
    {
        ...
        $thumbsDir = $webRoot . '/images/user/' . $userId . '/send/thumbs/';

        $resizedImagesDir = $webRoot . '/images/user/' . $userId . '/send/resized/';

        $imagesDir = $webRoot . '/images/user/' . $userId . '/send';

        $userImageCount = UserImageSend::find()->where(['user_id' => $userId])->count();
        ...
        $maxId = UserImageSend::find()->select('id')
            ->where(
                [
                    'user_id' => $userId,
        ...
        $key = md5(rand(1000, 9999999));

        $image_d = $imagesDir . $userId . '_' . $key. '_' . $imageIndex . ".jpg";

        $image_t = $thumbsDir . $userId . '_'. $key.'_' . $imageIndex . ".jpg";


        $saved = $image->saveAs($image_d);

        $imgObj = new UserImageSend();

        $imgObj->user_id = $userId;


        $imgObj->path_original = '/images/user/' . $userId . '/send/' . $userId .'_'.$key. '_' . $imageIndex . ".jpg";
        $imgObj->path = '/images/user/' . $userId . '/send/thumbs/' . $userId . '_'.$key.'_' . $imageIndex . ".jpg";
        ...
        if ($saved && $imgObj->primaryKey) {
            ...
            UserImage::resizeImageAndSave($image_d, $image_t, 120, 120);
