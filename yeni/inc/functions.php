...
function getOnline($type){
	...
	if((filemtime($tmpfile)+3) < time()){
		$cnt_all = mysql_result(mysql_query("SELECT COUNT(`id`) FROM `chat_users` WHERE `time` >= ".time()." AND `dating` = 0;"), 0);
		$cnt_women = mysql_result(mysql_query("SELECT COUNT(`id`) FROM `chat_users` WHERE `time` > ".time()." AND `dating` = 0 AND `sex`=1;"), 0);
...
function updateXal(){
	...
	if($data[0] != date('Y-m-d')){
		mysql_query("UPDATE `admin_alochat`.`user` SET `point` = '0' WHERE `point` > 0;");
		mysql_query("UPDATE `chat_users` SET `point` = '0' WHERE `point` > 0;");
...
function updatePosts(){
	...
	if($data[0] != date('Y-m-d')){
		mysql_query("UPDATE `admin_alochat`.`user` SET `msg_count_day` = '0' WHERE `msg_count_day` > 0;");
		mysql_query("UPDATE `chat_users` SET `post_day` = '0' WHERE `post_day` > 0;");
		if(mysql_affected_rows()>0){
			$fp = fopen($tempfile, 'w');
			fwrite($fp, date('Y-m-d'));
			fclose($fp);
...
function checkPhoneBan($phone){
	$query = mysql_query("SELECT `reason` FROM `chat_phone_ban` WHERE `phone` = '".$phone."'");
