...
$shareQuery = mysql_query("SELECT `id`, `user_id` as uid, `text`, `attach`, `permission`,`like_count` as `likes`,`time` as `date` FROM admin_alochat.`share` WHERE `user_id` = '".$_uid."' ".$ins_permission." ORDER BY `date` DESC LIMIT 3;");
...
if(mysql_num_rows($shareQuery)>0){
	...
	while($row = mysql_fetch_array($shareQuery)){
		...
		$count_comms = mysql_result(mysql_query("SELECT COUNT(`id`) FROM `admin_alochat`.`share_comment` WHERE `sid` = '".$shareId."';"), 0);
