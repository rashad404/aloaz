...
if(!isset($_POST['submit']) && !isset($_GET['page'])){
	...
	$max_yash=70;
	 for ($min_yash=12; $min_yash<=$max_yash; $min_yash++){
		...
		echo '<option value="'.$min_il.'">'.$min_yash.'</option>';
	}
	...
	for($min_yash=12; $min_yash<=$max_yash; $min_yash++){
		...
		echo '<option value="'.$min_il.'">'.$min2_yash.'</option>';
	}
	echo '</select><br/>';
...
if($_SESSION['advanced'] > 0){
	$ins_sql = " `il` <= '".$_SESSION['search_year1']."' AND `il` >= '".$_SESSION['search_year2']."' ";
	if($_SESSION['search_sex'] != 100) $ins_sql .= " AND `sex` = '".$_SESSION['search_sex']."'";
	if($_SESSION['search_on'] == 'on') $ins_sql .= " AND `time` > ".(time()-$online_time).""; else $ins_sql .= " AND `time` < ".(time()-$online_time)."";
	if($_SESSION['search_photo'] == 'on') $ins_sql .= " AND `photo` != ''";
}
else{
...
$sql = mysql_query("SELECT COUNT(`id`) FROM `chat_users` WHERE ".$ins_sql.";");
...
$q = mysql_query("SELECT `id`, `nickname`, `sex`, `name`, `il`, `photo`, `time` FROM `chat_users` WHERE ".$ins_sql." ORDER BY `time` DESC LIMIT ".$start.", ".$show_limit.";");
while($users = mysql_fetch_array($q)){
	$users_login = $users['nickname'];
	$users_id = $users['id'];
	$users_photo= $users['photo'];
	$users_name= $users['name'];
	$users_byear = $users['il'];
	$users_sex = $users['sex'];
	$users_time = $users['time'];
	
	$expPhoto = explode('|', $users_photo);
	$photoId = $expPhoto[1];
	...
	$users_age = date('Y')-$users_byear;
	
	if($users_time > time()) $onlineInfo = '<span style="font-size:11px; color: green;">'.$__lng['onlayn'].'</span>'; else $onlineInfo = '<span style="font-size:11px;">'.date('Y-m-d H:i', ($users_time-600)).'</span>';
	
	if(empty($users_photo)) $img_file = 'img/'.$users_sex_img.'.gif';
	else $img_file = 'photos/preview.php?photo_id='.$photoId.'';
	
	echo '<tr '; echo $i++ % 2 ? ' style="background: #f6f4f4"' : ''; echo '><td><img src="'.$img_file.'" alt="man" style="border: 1px solid #d7d7d7" /></td>
