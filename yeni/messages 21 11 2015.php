...
$checkAuth = checkAuth('`id`, `nickname`, `phone`, `hhh`, `friends`, `post_run`, `post_day`');
if($checkAuth == 'error'){
...
$to_sql = mysql_query("SELECT `nickname`, `friends`, `dating` FROM `chat_users` WHERE `id` = '".$toid."';");
...
if(mysql_affected_rows() == 0){
	...
	mysql_query("UPDATE admin_alochat.`conversation_reply` SET `read` = 1, `read_time` = '".time()."' WHERE `user_id_to` = '".$id."' AND `user_id` = '".$toid."';");
	
	$user_data = mysql_fetch_array($to_sql);
	$uid_friends = $user_data['friends'];
	$uid_dating = $user_data['dating'];
	$to_nick = $user_data['nickname'];
	$to_love = $user_data['love'];
}

if($_del > 0 && $_mid > 0){
	$query = mysql_query("SELECT `user_id`, `user_id_to`,`deleted_by` FROM admin_alochat.`conversation_reply` WHERE `id` = '".$_mid."';");
	...
	if($deleted_by>0 && $deleted_by!=$id){
		mysql_query("DELETE FROM admin_alochat.`conversation_reply` WHERE `id` = '".$_mid."';");
	}else{
		$del_update = mysql_query("UPDATE admin_alochat.`conversation_reply` SET `deleted_by` = '".$id."' WHERE `id` = '".$_mid."';");
	}
}

if(isset($_POST['action']) && mysql_num_rows($to_sql) > 0 && !empty($message)){
...
if($_POST['action'] != 'sendattach'){
	$q = mysql_query("SELECT `id` FROM admin_alochat.`conversation_reply` WHERE `reply` = '".$message."' AND `user_id_to` = '".$toid."' AND `user_id` = '".$id."' AND `time` > '".(time()-300)."';");
...
$checkFriendQuery = mysql_query("SELECT `ok1`, `ok2` FROM `chat_friends` WHERE (`id` = '".$toid."' AND `uid` = '".$id."') OR (`uid` = '".$toid."' AND `id` = '".$id."');");
$checkFriendRow = mysql_fetch_array($checkFriendQuery);
$ok1 = $checkFriendRow['ok1'];
$ok2 = $checkFriendRow['ok2'];

if($uid_friends == 0 && mysql_num_rows($checkFriendQuery) == 0){
	echo "<b>".$__lng['dostlardan mesaj qebul edir']."</b><br/>";
	echo '<br/><a href="javascript:history.back(1)">« '.$__lng['geri'].'</a><br/>';
	break;
}

if($uid_friends == 0 && ($ok1 != 1 || $ok2 != 1)){
...
$checkBlockQuery = mysql_query("SELECT `id` FROM `chat_blocks` WHERE `id` = '".$toid."' AND `uid` = '".$id."';");
...
if($_POST['action'] == 'sendattach'){
	...
	$check_attach_limit = mysql_query("SELECT `id` FROM admin_alochat.`conversation_reply` WHERE `user_id` = '".$id."' AND `attach` != '' AND `time` > '".strtotime(date('Y-m-d 00:00:00'))."';");
	...
	if(move_uploaded_file($_FILES[attach_file][tmp_name], 'attach/data/temp/'.$store_name.'')){
		...
		if(mysql_num_rows($check_attach_limit) > 10 && $point > 0) mysql_query("UPDATE `chat_users` SET `hhh` = `hhh` - 1 WHERE `id` = '".$id."' LIMIT 1;"); // fee
		$message = $__lng['yukle'].':';
	}
}

$conversation_sql = mysql_query("SELECT id FROM admin_alochat.`conversation` WHERE (`user_one`='".$id."' and `user_two`='".$toid."') or (`user_one`='".$toid."' and `user_two`='".$id."')");
if(mysql_num_rows($conversation_sql) == 0){
	$insert_conversation = mysql_query("INSERT INTO admin_alochat.`conversation` SET `user_one`='".$id."',`user_two`='".$toid."',`last_time`='".time()."',`last_reply`='".$message."'");
	$conversation_id = mysql_insert_id();
}else {
	$conversation = mysql_fetch_assoc($conversation_sql);
	$conversation_id = $conversation["id"];
	mysql_query("UPDATE admin_alochat.`conversation` SET `last_time`='".time()."',`last_reply`='".$message."' WHERE `id`='".$conversation_id."'");
}
$insert = mysql_query("INSERT INTO admin_alochat.`conversation_reply` SET `conversation_id`='".$conversation_id."',`user_id` = '".$id."', `user_id_to` = '".$toid."', `from_nick` = '".$login."', `reply` = '".$message."', `time` = '".time()."' ".$ins_attach.";");
...
if(!$insert){
	...
	if(mysql_num_rows($checkFriendQuery) > 0){
		mysql_query("UPDATE `chat_friends` SET `act_time` = '".time()."' WHERE (`id` = '".$toid."' AND `uid` = '".$id."') OR (`uid` = '".$toid."' AND `id` = '".$id."') LIMIT 1;");
	}
}
...
$new_messages = mysql_query("SELECT COUNT(`id`) FROM admin_alochat.`conversation_reply` WHERE `user_id_to` = '".$id."' AND `read` = 0");
...
if($toid != 1){
	$checkFriendQuery = mysql_query("SELECT `id` FROM `chat_friends` WHERE (`id` = '".$toid."' AND `uid` = '".$id."') OR (`uid` = '".$toid."' AND `id` = '".$id."');");
	if($uid_friends == 0 && mysql_num_rows($checkFriendQuery) == 0){
		echo $__lng['yalniz dostlardan msj q e'].'.<br/><br/>';
	}
	else{
	echo '<form action="messages.php?mod=messaging&amp;uid='.$toid.'&amp;back='.$_back.'" method="post">';
...
$row_count = mysql_query("SELECT COUNT(`id`) FROM admin_alochat.`conversation_reply` WHERE ((`user_id` = '".$id."' AND `user_id_to` = '".$toid."' AND `deleted_by` != '".$id."') OR (`user_id` = '".$toid."' AND `user_id_to` = '".$id."' AND `deleted_by` != '".$id."'));");
...
$query = mysql_query("SELECT `id`, `reply`, `time`, `user_id`, `from_nick`, `user_id_to`, `read`, `attach` FROM admin_alochat.`conversation_reply` WHERE ((`user_id` = '".$id."' AND `user_id_to` = '".$toid."' AND `deleted_by` != '".$id."') OR (`user_id` = '".$toid."' AND `user_id_to` = '".$id."' AND `deleted_by` != '".$id."')) ORDER BY `id` DESC LIMIT ".$start.", ".$show_limit.";");
...
if(isset($_GET['delete'])){
	$uid = intval($_GET['uid']);
	$q = mysql_query("DELETE FROM admin_alochat.`conversation_reply` WHERE `user_id_to` = '".$id."' AND `user_id` = '".$uid."' AND `read` = '0';");
	if(mysql_affected_rows() > 0){
...
if($count_unread == 0){
	...
	$all_rows = mysql_num_rows(mysql_query("SELECT `id` FROM admin_alochat.`conversation_reply` WHERE `user_id_to` = '".$id."' AND `read` = 0 GROUP BY `user_id`"));
	...
	$unread_query = mysql_query("SELECT `from_nick`, `user_id`, COUNT(`user_id`), `time` FROM admin_alochat.`conversation_reply` WHERE `user_id_to` = '".$id."' AND `read` = 0 GROUP BY `user_id` ORDER BY time DESC LIMIT ".$start.", ".$show_limit."");
...
$recentQuery = mysql_query("SELECT `user_id`, `user_id_to`, `time`, `read` FROM admin_alochat.`conversation_reply` WHERE (`user_id` = '".$id."' OR `user_id_to` = '".$id."') AND `deleted_by`!='".$id."' ORDER BY `time` DESC LIMIT 2000;");
...
foreach($recentArray as $date => $value){
	...
	foreach($recentArray[$date] as $rId => $value){
		...
		$rNick = mysql_result(mysql_query("SELECT `nickname` FROM `chat_users` WHERE `id` = '".$rId."';"), 0);
...
$unread_count = mysql_query("SELECT COUNT(`id`) FROM admin_alochat.`conversation_reply` WHERE `user_id_to` = '".$id."' OR `user_id` = '".$id."'");
$count_unread = mysql_result($unread_count, 0);
...
if($count_unread == 0){
	...
	$all_rows = mysql_num_rows(mysql_query("SELECT `id` FROM admin_alochat.`conversation_reply` WHERE `user_id_to` = '".$id."' OR `user_id` = '".$id."' GROUP BY `user_id`"));
...
case 'clear':
if($_GET['confirm'] == 'ok'){
	mysql_query("DELETE FROM admin_alochat.`conversation_reply` WHERE `user_id_to` = '".$id."' AND `read` = '0';");
	if(mysql_affected_rows() > 0){
