...
if(isset($_POST['submit'])){	
	...
	if(!empty($error)){
		...
		if(!empty($_name)) $ins_sql .= " `name` = '".$_name."', ";
...
//		if(!empty($_height))
		...
		if(!empty($_country)) $ins_sql .= " `country` = '".$_country."', ";
		
		mysql_query("UPDATE `chat_users` SET ".$ins_sql." `time` = '".(time()+600)."' WHERE `id` = '".$id."' LIMIT 1;");
...
echo '<a href="profile_edit1.php?mod=photo">'.$__lng['shekil elave et'].'</a> | ';
echo '<a href="profile_edit1.php?mod=pass">'.$__lng['shifre deyish'].'</a><br/><br/>';

echo '<form action="profile_edit1.php" method="POST">';
...
if(isset($_GET['del'])){
	if($_GET['del'] != 1){
		...
		echo '<a href="profile_edit1.php?mod=photo&amp;del=1">'.$__lng['beli sil'].'</a><br/><br/>';
	}
	else{
		...
		if($unlink1 || $unlink2){
			...
			mysql_query("UPDATE `chat_users` SET `photo` = '' WHERE `id` = '".$id."' LIMIT 1;");
			$delSuccess = true;
		}
	}
}
...
if(!isset($_POST['action'])){
	if(!empty($photo) && !$delSuccess){
		...
		if(!isset($_GET['del'])) $del_link = ' <a href="profile_edit1.php?mod=photo&amp;del=">'.$__lng['sekli sil'].'</a>';
		echo '<img src="'.$img_file.'" alt="man" style="border: 1px solid #d7d7d7" /> '.$del_link.'<br/><br/>';
	}
	
	echo '<b>'.$__lng['qeyri etik shekil olmaz'].'</b><br/>'.$__lng['eks halda girise qadaga'].'<br/><br/>';
	echo $__lng['shekil sech'].':<br/>';
	echo '<form action="profile_edit1.php?mod=photo" method="post" enctype="multipart/form-data">';
	...
	mysql_query("UPDATE `chat_ozunutanit` SET `photo_id` = '".$photo_ins_id."' WHERE `uid` = '".$id."';");
...
case 'pass':
 if(!isset($_POST['submit'])){
	echo '<form action="profile_edit1.php?mod=pass" method="POST">';
	...
	echo strlen($_new_pass); exit;
	mysql_query("UPDATE `chat_users` SET `password` = '".$_new_pass."', `md5_pass` = '".md5($_new_pass)."' WHERE `id` = '".$id."' AND `password` = '".$_old_pass."' LIMIT 1;");
	mysql_query("UPDATE `admin_alochat`.`user` SET `changed_pass` = 1 WHERE `id` = '".$id."' LIMIT 1;");
