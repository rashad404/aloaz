...
$id = $userrow['id'];
$a_user = mysql_fetch_assoc(mysql_query("select id,nickname,password from `aloaz_db`.`user` where id='".$id."'"));
 if($a_user and $a_user["nickname"]==$userrow["nickname"] and $a_user["password"]==$userrow["password"]){

	$user_log = mysql_fetch_assoc(mysql_query("select id from `aloaz_db`.`user_logs` where user_id='".$id."' and login_status=0 and login_time=0"));
	if($user_log){

		$rand = rand("1000","9999");
		mysql_query("UPDATE `aloaz_db`.`user_logs` SET rand ='".$rand."' where id='".$user_log["id"]."'");
		...
		echo "saas"; 
		$rand = rand("1000","9999");
		$insert = mysql_query("INSERT INTO `aloaz_db`.`user_logs` SET `user_id`='".$id."',`rand`='".$rand."',`login_status`=0,`login_time`=0");
		if($insert){
			$code = md5(md5($rand."aloaz123".$userrow["id"].$userrow["nickname"].$userrow["password"].$userrow["id"]."alochat456".$rand));
