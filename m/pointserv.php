...
function checkCoinRequests(){
	global $id, $point, $login;
	$_request_var = json_encode($_REQUEST);
	
	$getCoinQuery = mysql_query("SELECT SUM(`coins`) AS `sum_coins` FROM `aloaz_db`.`coin_logs` WHERE `user_id` = '".$id."' AND `type` = 2");
	$getCoinRow = mysql_fetch_array($getCoinQuery);
	$sum_coins = $getCoinRow['sum_coins'];
	
	if($point > $sum_coins){
		mysql_query("INSERT INTO `aloaz_db`.`fake_coin_users` SET `user_id` = '".$id."', `login` = '".$login."', `sum_coins` = '".$sum_coins."', `coins` = '".$point."', `request` = '".$_request_var."', `time` = '".time()."'");
...
$simaz->connect();
$error = null;
if($_POST['request'] == 'azercell') {
	$bal_tariffs = $simaz->array_numeric($simaz->params['bal_tariffs']);
	$azn = $_POST['azn'];
	if(!is_numeric($azn)) {
		$error = 'Məlumat düzgün seçilməyib';
	} else if(!$bal_tariffs[$azn]){
		$error = 'Məlumat düzgün seçilməyib';
	}
	if( !$error ) {
		$checkAuth = checkAuth('`id`');
		$userrow = mysql_fetch_assoc($checkAuth);

		$simaz->get('bal', array('userid' => $userrow['id'], 'bal' => $bal_tariffs[$azn], 'amount' => $azn));
	}
}
...
echo '- <a href="pointserv.php?mod=renglinik">'.$__lng['rengli nik'].'</a><br/>';
echo '- <a href="pointserv.php?mod=onlinerating">'.$__lng['irelide gorun'].'</a><br/>';
echo '- <a href="online_message.php">Onlayn Mesaj</a><br/>';
...
echo 'Bal almaq üçün admin <a href="/profile.php?uid=1111">KUTAY</a> nikine müraciet edin.<br/>';
//echo 'Aşağıdakı üsullar vasitəsi ilə bal ala bilərsiniz:<br/>';
//echo '<br />
//<a class="payButton" href="pointserv.php?mod=pm_azercell" style="background-color: #818181; color: white; padding: 4px; border: 0px; font-size: 14px;">Azercell Şifrə ilə Bal Yüklə</a>
//<br/><br/>
...
case 'pm_azercell';
	...
	echo '<div align="center">';
		echo "<b>Sim Konturla Bal Y&#252;kle</b> (Avtomatik)<br/>\n";
	echo '</div>';
	echo '<br/>';
	...
	echo '<form id="azercell-checkout" action="pointserv.php?mod=pm_azercell" method="post">';
	echo "Qiymet (se&#231;):<br/>\n";
	$option =  "<select name=\"azn$ref\">|";
	foreach( $bal_tariffs as $price => $valueBal) {
		$option .= "<option value=\"{$price}\">{$price} AZN ({$valueBal} bal)</option>|";
	}
	$option .= "</select>";
	echo $option.'<br/>';
	echo '<input type="hidden" name="request" value="azercell">';
...
?>
	...
	<form action='https://portmanat.az/checkout' method='post' id="portmanat-checkout" style="display: none">
...
if(!isset($_POST['action'])){
	if($rengli_row && !empty($rengli_row["file"]) && !$delSuccess){
		...
		if(!isset($_GET['del'])) $del_link = ' <a href="pointserv.php?mod=renglinik&amp;del=0">Sil</a>';
...
if($id != 1111){
    echo 'Bal göndermek müveqqeti olaraq dayandırılmışdır.<br/>';
    break;
}
...
echo 'Bu funksiya müveqqeti olaraq dayandırılmışdır.<br/>';
break;
