...
if(mysql_affected_rows() == 0){
	...
	mysql_query("UPDATE `chat_messages` SET `read` = 1, `read_time` = '".time()."' WHERE `to` = '".$id."' AND `from` = '".$toid."';");
...
if($_del > 0 && $_mid > 0){
	$query = mysql_query("SELECT `to`, `from` FROM `chat_messages` WHERE `lid` = '".$_mid."';");
	...
	if($to == $id) $del_update = mysql_query("UPDATE `chat_messages` SET `del2` = '1',`deleted_by` = '".$id."' WHERE `lid` = '".$_mid."' AND `to` = '".$id."';");
	if($from == $id) $del_update = mysql_query("UPDATE `chat_messages` SET `del1` = '1',`deleted_by` = '".$id."' WHERE `lid` = '".$_mid."' AND `from` = '".$id."';");
 	//mysql_query("DELETE FROM `chat_messages` WHERE `lid` = '".$_mid."' AND `del1` != 0 AND `del2` != 0 AND `time` < '".(time() - $old_msg*86400)."';");
}
...
if($_POST['action'] != 'sendattach'){
	$q = mysql_query("SELECT `lid` FROM `chat_messages` WHERE `message` = '".$message."' AND `to` = '".$toid."' AND `from` = '".$id."' AND `time` > '".(time()-300)."';");
...
if($_POST['action'] == 'sendattach'){
	...
	$check_attach_limit = mysql_query("SELECT `lid` FROM `chat_messages` WHERE `from` = '".$id."' AND `attach` != '' AND `time` > '".strtotime(date('Y-m-d 00:00:00'))."';");
...
$conversation_sql = mysql_query("SELECT id FROM `conversation` WHERE (`user_one`='".$id."' and `user_two`='".$toid."') or (`user_one`='".$toid."' and `user_two`='".$id."')");
if(mysql_num_rows($conversation_sql) == 0){
	$insert_conversation = mysql_query("INSERT INTO `conversation` SET `user_one`='".$id."',`user_two`='".$toid."',`last_time`='".time()."',`last_reply`='".$message."'");
	$conversation_id = mysql_insert_id();
}else {
	$conversation = mysql_fetch_assoc($conversation_sql);
	$conversation_id = $conversation["id"];
}
$insert = mysql_query("INSERT INTO `chat_messages` SET `conversation_id`='".$conversation_id."',`from` = '".$id."', `to` = '".$toid."', `from_nick` = '".$login."', `message` = '".$message."', `time` = '".time()."' ".$ins_attach.";");
mysql_query("UPDATE `conversation` SET `last_time`='".time()."',`last_reply`='".$message."' WHERE `id`='".$conversation_id."'");
if(!$insert){
...
$new_messages = mysql_query("SELECT COUNT(`lid`) FROM `chat_messages` WHERE `to` = '".$id."' AND `read` = 0");
...
$row_count = mysql_query("SELECT COUNT(`lid`) FROM `chat_messages` WHERE ((`from` = '".$id."' AND `to` = '".$toid."' AND `del1` = '0') OR (`from` = '".$toid."' AND `to` = '".$id."' AND `del2` = '0'));");
...
$query = mysql_query("SELECT `lid`, `message`, `time`, `from`, `from_nick`, `to`, `read`, `attach` FROM `chat_messages` WHERE ((`from` = '".$id."' AND `to` = '".$toid."' AND `del1` = '0') OR (`from` = '".$toid."' AND `to` = '".$id."' AND `del2` = '0')) ORDER BY `lid` DESC LIMIT ".$start.", ".$show_limit.";");
...
if(isset($_GET['delete'])){
	$uid = intval($_GET['uid']);
	$q = mysql_query("DELETE FROM `chat_messages` WHERE `to` = '".$id."' AND `from` = '".$uid."' AND `read` = '0';");
	if(mysql_affected_rows() > 0){
...
if($count_unread == 0){
	...
	$all_rows = mysql_num_rows(mysql_query("SELECT `lid` FROM `chat_messages` WHERE `to` = '".$id."' AND `read` = 0 GROUP BY `from`"));
	...
	$unread_query = mysql_query("SELECT `from_nick`, `from`, COUNT(`from`), `time` FROM `chat_messages` WHERE `to` = '".$id."' AND `read` = 0 GROUP BY `from` ORDER BY time DESC LIMIT ".$start.", ".$show_limit."");
	while($unread_row = mysql_fetch_array($unread_query)){
		$from_nick = $unread_row['from_nick'];
		$from = $unread_row['from'];
		$time = $unread_row['time'];
		$c_unread = $unread_row['COUNT(`from`)'];
		
		if(date("d-m-Y", $time) == date("d-m-Y")) $print_date = date("H:i", $time);
		else $print_date = date("d-m-Y H:i", $time);
		
		//$count_unread_q = mysql_query("SELECT COUNT(`lid`) FROM `chat_messages` WHERE `from` = '".$from."' AND `to` = '".$id."' AND `read` = '0' ;");
...
$recentQuery = mysql_query("SELECT `from`, `to`, `time`, `read` FROM `chat_messages` WHERE (`from` = '".$id."' AND `del1` = '0') OR (`to` = '".$id."' AND `del2` = '0') ORDER BY `time` DESC LIMIT 2000;");
...
$unread_count = mysql_query("SELECT COUNT(`lid`) FROM `chat_messages` WHERE `to` = '".$id."' OR `from` = '".$id."'");
$count_unread = mysql_result($unread_count, 0);
...
if($count_unread == 0){
	...
	$all_rows = mysql_num_rows(mysql_query("SELECT `lid` FROM `chat_messages` WHERE `to` = '".$id."' OR `from` = '".$id."' GROUP BY `from`"));
...
case 'clear':
if($_GET['confirm'] == 'ok'){
	mysql_query("DELETE FROM `chat_messages` WHERE `to` = '".$id."' AND `read` = '0';");
	if(mysql_affected_rows() > 0){
