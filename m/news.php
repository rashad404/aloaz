...
echo '<div class="mnav">'.$home_alochat.' » <a href="news.php">'.$title.'</a></div>';
...
$query = mysql_query("SELECT `id`, `title`, `body`, `time` FROM `aloaz_db`.`news` ORDER BY `time` DESC LIMIT ".$start.", ".$show_limit.";");
while($row = mysql_fetch_array($query)){
	...
	$cnt_news = mysql_result(mysql_query("SELECT COUNT(`id`) FROM `aloaz_db`.`news_comment` WHERE `news_id` = '".$news_id."'"), 0);
	
	echo '<span style="font-size: 12px">'.date('d-m-Y H:i', $news_time).' <img src="img/comment.png" alt="." style="vertical-align:middle; padding: 0 2px 0 5px" /> '.$cnt_news.'</span><br/>';
...
if($admin_status == 1){
	...
	if($_del == 1){
		...
		echo '<a href="news.php?mod=read&id='.$_news_id.'&amp;page='.$page.'">'.$__lng['xeyr'].'</a> / ';
		echo '<a href="news.php?mod=read&id='.$_news_id.'&amp;commentid='.$_commentid.'&amp;page='.$page.'&amp;del=2">'.$__lng['beli'].'</a><br/>';
		echo '</div>';
	}
	if($_del == 2){
...
$news_time = $row['time'];
$news_body = str_replace(array_keys($smilesArray), array_values($smilesArray), $news_body);

echo '<span style="font-size: 12px">'.date('d-m-Y H:i', $news_time).'</span><br/>';
...
while($row = mysql_fetch_array($query)){
	...
	if($admin_status == 1) echo '<span style="float:right; padding-right: 8px;"><a href="news.php?mod=read&id='.$_news_id.'&amp;page='.$page.'&amp;del=1&amp;commentid='.$comment_id.'">'.$__lng['sil'].'</a></span>';
