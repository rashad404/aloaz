...
echo '<p align="center"><b>'.$roomName.'</b> otağı <img src="/img/online.png" alt="on" /><span style="color: green">'.$countRoomOnline.'</span><br/>';
if($roomType==0){
...
$gizli_message = false;
$gizli_message_button = 'Gizli';
$gizli_message_url = 'msgs.php?rid='.$_rid.'&gizli=1';
if(isset($_GET["gizli"]) and intval($_GET["gizli"])==1){
	$gizli_message = true;
	$where_user = "(`to` = '".$id."' OR (`uid` = '".$id."' and `to`>0))" ; 
	$gizli_message_button = 'Ümumi';
	$gizli_message_url = 'msgs.php?rid='.$_rid.'';
}


echo '<form action="msgs.php?rid='.$_rid.'" method="post" name="room-form">';
echo '<a href="/smiles.php"><img src="/img/smiles/1f603.png" alt="Smile" /></a> <input type="text" name="message" id="id_box"  /> ';
echo '<input type="submit" name="submit" value="'.$__lng['yaz'].'" class="submitButton" /> <a class="button" href="msgs.php?rid='.$_rid.'">'.$__lng['yenile'].'</a> <a class="button" href="'.$gizli_message_url.'">'.$gizli_message_button.'</a><br/>';
