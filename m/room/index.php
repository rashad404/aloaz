...
//echo 'Viktorina və daimi otaqlar istifadəyə verildi!<br/><br/>';

$all_rows = mysql_query("SELECT COUNT(`id`) FROM `aloaz_db`.`room` WHERE `type`=1 and id!=10");
...
*/
	...
	$t   = time()-600;
	$usersQuery = mysql_query("SELECT `nickname` FROM `aloaz_db`.`user` WHERE `place` = '10' AND `last_activity` > '".$t."' ORDER BY `last_activity` DESC");
	$roomOnline = mysql_num_rows($usersQuery);
	
	mysql_query("UPDATE `aloaz_db`.`room` SET `online` = '".$roomOnline."' WHERE `id` = '".$room_id."' LIMIT 1");
	echo '<div style="margin-bottom:8px"><a href="viktorina.php">Viktorina otağı</a> ('.$roomOnline.') <br/>';//# <span style="color: gray">'.$room_login.'</span>
	while($usersRow = mysql_fetch_array($usersQuery)){
		$usersLogin = $usersRow['nickname'];
		$roomUsers .= $usersLogin.', ';
	}
...
$query = mysql_query("SELECT `id`, `name`, `uid`, `login` FROM `aloaz_db`.`room` WHERE `type`=1 and id!=10 ORDER BY `position` ASC,`id` ASC LIMIT ".$start.", ".$show_limit.";");
while($row = mysql_fetch_array($query)){
	$room_id = $row['id'];
	$room_name = $row['name'];
	$room_login = $row['login'];
	$t   = time()-600;
	$usersQuery = mysql_query("SELECT `nickname`,`id` FROM `aloaz_db`.`user` WHERE `place` = '".$room_id."' AND `last_activity` > '".$t."' ORDER BY `id` ASC");
	$roomOnline = mysql_num_rows($usersQuery);
	
	mysql_query("UPDATE `aloaz_db`.`room` SET `online` = '".$roomOnline."' WHERE `id` = '".$room_id."' LIMIT 1");
	
	echo '<div style="margin-bottom:8px"><a href="msgs.php?rid='.$room_id.'">'.$room_name.' otağı</a> ('.$roomOnline.') <br/>';//# <span style="color: gray">'.$room_login.'</span>
	...
	echo '<br />';
	echo '<a href="private.php">Şexsi otaqlar</a> <br/>';//# <span style="color: gray">'.$room_login.'</span>
