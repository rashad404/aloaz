...
if(mysql_num_rows($viktorina_query)>0){
	...
	if(($viktorina["help_time"]+$paramsArray["viktorina_help_time"])<time()){ 
		...
		if($help_count<strlen($viktorina["answer"])-2){
...
echo '<a href="viktorina-top.php?period=all">Viktorina reytinqi</a><br/>';
...
echo '<form action="viktorina.php" method="post">';
		...
		if(!$error){
			...
			if(mysql_affected_rows() > 0){
				if(strtolower($viktorina["answer"]) == strtolower(trim($_message))){				
					$q_msgs = 'Tebrikler <b>'.$login.'</b>! Cavab doğrudur. Növbeti sual '.$paramsArray["viktorina_question_time"].' saniye sonra...';
...
if(mysql_num_rows($query) == 0){
	...
	echo '<br/>';
	if($page > 1) echo '<a id="pageButon" href ="viktorina.php?page='.($page-1).'">&lt;</a> ';
	if($page < $max) echo '<a id="pageButon" href ="viktorina.php?page='.($page+1).'">&gt;</a>';
