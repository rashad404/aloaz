...
$roomQuery = mysql_query("SELECT `id`, `name` FROM `room` WHERE `uid` = '".$id."';");
...
if($_submit > 0 && $_uid > 0){
	$update = mysql_query("UPDATE `room_subs` SET `status` = '".$_submit."' WHERE `rid` = '".$rid."' AND `uid` = '".$_uid."'");
	if(!$update) echo 'DB error<br/>';
}

$all_rows = mysql_query("SELECT COUNT(`id`) FROM `room_subs` WHERE `status` = '".$_status."' AND `rid` = '".$rid."'");
...
$query = mysql_query("SELECT `uid`, `time` FROM `room_subs` WHERE `status` = '".$_status."' AND `rid` = '".$rid."' ORDER BY `time` DESC LIMIT ".$start.", ".$show_limit.";");
...
if(mysql_num_rows($query) == 0){
	...
	while($row = mysql_fetch_array($query)){
		...
		$subsQuery = mysql_query("SELECT `nickname` FROM `chat_users` WHERE `id` = '".$subs_uid."'");
...
$roomQuery = mysql_query("SELECT `id`, `name`, `view` FROM `room` WHERE `uid` = '".$id."'");
if(mysql_num_rows($roomQuery) == 0){
	echo $__lng['otaq tapilmadi'].'.<br/>';
	echo '</div>';
	include '../inc/footer.php';
...
if(isset($_POST['submit']) ){
	...
	$checkNameQuery = mysql_query("SELECT `id` FROM `room` WHERE `name` = '".$_name."' AND `id` != '".$roomId."' LIMIT 1");
	...
	if(!empty($error)){
		...
		$update = mysql_query("UPDATE `room` SET `name` = '".$_name."', `view` = '".$_view."' WHERE `id` = '".$roomId."' LIMIT 1");
		if($update){
...
$delete = mysql_query("DELETE FROM `room_msgs` WHERE `rid` = '".$rid."'");
...
mysql_query("DELETE FROM `room` WHERE `id` = '".$rid."' AND `uid` = '".$id."'");
if(mysql_affected_rows() > 0){
	echo $__lng['otaq silindi'].'<br/>';
	mysql_query("DELETE FROM `room_subs` WHERE `rid` = '".$rid."'");
}
else echo 'DB error<br/>';
...
$subsQuery = mysql_query("SELECT status, COUNT(id) FROM `room_subs` WHERE `rid` = '".$rid."' GROUP BY status"); 
