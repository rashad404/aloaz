...
if(isset($_POST['submit']) ){
	...
	$checkLimitQuery = mysql_query("SELECT `id` FROM `room` WHERE `uid` = '".$id."' LIMIT 1");
	if(mysql_num_rows($checkLimitQuery) > 0) $error .= '- '.$__lng['otaq limiti sehvi'].'<br/>';
	
	$checkNameQuery = mysql_query("SELECT `id` FROM `room` WHERE `name` = '".$_name."' LIMIT 1");
	...
	if(!empty($error)){
		...
		mysql_query("INSERT INTO `room` SET `name` = '".$_name."', `uid` = '".$id."', `login` = '".$login."', `view` = '".$_view."', `country` = '".$country."', `time` = '".time()."'");
