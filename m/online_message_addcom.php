...
echo '<div class="mnav"><a href="/main.php">AloChat</a> » <a href="online_message.php?mod=read&amp;id='.$_id.'">'.$__lng['sherhler'].'</a></div>';
echo '<div class="layer">';

$messageQuery = mysql_query("SELECT `id`, `user_id` FROM aloaz_db.`online_message` WHERE `id` = '".$_id."';");
...
$message_uid = $messageRow['user_id'];

$checkAuth = checkAuth('`id`, `phone`, `mysmile`, `nickname`');
if($checkAuth == 'error'){
...
$auth_nickname = $userrow["nickname"];
...
if(!isset($_POST['secnumber'])){
	...
	echo $__lng['sherh'].' (maks: 300 simvol):<br/>';
	...
	$checkLimit = mysql_result(mysql_query("SELECT COUNT(`id`) FROM aloaz_db.`online_message_comment` WHERE `user_id` = '".$uid."' AND `message_id` = '".$_id."' AND `time` > '".(time()-60)."'"), 0);
	if($checkLimit > 5){
		$error .= '- Spam!!!<br/>';
		mysql_query("INSERT INTO `aloaz_db`.`blocks` SET `from_id` = '1', `user_id` = '".$uid."', `time`='10800',`blocked_time` = '".time()."',`ip`='".$block_ip."',`ua`='".$block_ua."',`reason`='3',`description`='Spam (Avtomatik)';");
		mysql_query("UPDATE `aloaz_db`.`user` SET `block_time`='10800',`block_begin_time`=0 WHERE id='".$uid."' LIMIT 1");
	}
	...
	mysql_query("INSERT INTO aloaz_db.`online_message_comment` SET `user_id` = '".$uid."', `message_id` = '".$_id."', `comment` = '".$comment."', `time` = '".time()."'");
	if(mysql_affected_rows() > 0){
		echo $__lng['muveffeqiyyetle elave olundu'].'<br/>';
		mysql_query("UPDATE aloaz_db.`online_message` SET `comments` = `comments` + 1 WHERE `id` = '".$_id."' LIMIT 1;");
		
		setNotification($message_uid,$paramsArray["NOTE_ONLINE_MESSAGE_COMMENT"],time(),$uid,$auth_nickname,0,$_id);
	}
	else{
		echo 'Error<br/>';
	}
