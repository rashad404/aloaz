...
$query = mysql_query("SELECT `id`, `nickname` FROM `aloaz_db`.`user` WHERE `id` = '".$_uid."';");
...
if(intval($_GET['cancel']) == 1){
	mysql_query("DELETE FROM `aloaz_db`.`chat_reports` WHERE `uid` = '".$_uid."' AND `reporter` = '".$id."'");
	if(mysql_affected_rows() > 0){
		echo $__lng['sikayet legv olundu'].'!<br/><br/>';
	}
}

$checkQuery = mysql_query("SELECT `id` FROM `aloaz_db`.`chat_reports` WHERE `uid` = '".$_uid."' AND `reporter` = '".$id."' AND `time` > '".(time()-3600*24*30)."'");
if(mysql_num_rows($checkQuery) > 0){
	...
	if($_POST['submit'] == ''){
		...
		elseif(strlen($_type) == ''){
			...
			mysql_query("INSERT INTO `aloaz_db`.`chat_reports` SET `uid` = '".$_uid."', `reporter` = '".$id."', `note` = '".$_note."', `type` = '".$_type."', `time` = '".time()."'");
			if(mysql_affected_rows() > 0){
