...
$all_rows = mysql_query("SELECT COUNT(`id`) FROM `aloaz_db`.`user_block` WHERE `block_from` = '".$id."'");
...
$query = mysql_query("SELECT `block_to` FROM `aloaz_db`.`user_block` WHERE `block_from` = '".$id."' ORDER BY `time` DESC LIMIT ".$start.", ".$show_limit.";");
...
$block_user = mysql_fetch_assoc($checkUserQuery);
if($block_user["user_status"]==10){
	echo "Siz bu istifadəçiyə qadağa qoya bilməzsiniz";
	break;
}


$checkBlockQuery = mysql_query("SELECT `block_from` FROM `aloaz_db`.`user_block` WHERE `block_from` = '".$id."' AND `block_to` = '".$_uid."';");
...
mysql_query("INSERT INTO `aloaz_db`.`user_block` SET `block_from` = '".$id."', `block_to` = '".$_uid."', `time` = '".time()."';");
mysql_query("UPDATE `aloaz_db`.`conversation` SET `blocked`=1 WHERE (user_one='".$id."' AND user_two='".$_uid."') OR (user_one='".$_uid."' AND user_two='".$id."') LIMIT 1");
...
mysql_query("DELETE FROM `aloaz_db`.`user_block` WHERE `block_from` = '".$id."' AND `block_to` = '".$_uid."' LIMIT 1;");

if(mysql_affected_rows()>0){
	echo $__lng['qadaga legv olundu'].'.<br/>';
	mysql_query("UPDATE `aloaz_db`.`conversation` SET `blocked`=0 WHERE (user_one='".$id."' AND user_two='".$_uid."') OR (user_one='".$_uid."' AND user_two='".$id."') LIMIT 1");
}
else{
	echo 'Database error<br/>';
}
