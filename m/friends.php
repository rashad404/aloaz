...
while($friendsRow = mysql_fetch_array($friendsQuery)){
	...
	$friendUserQuery = mysql_query("SELECT `nickname`, `sex`, `full_name`, `profile_photo`, `last_activity`, `age`,`last_post`,`invisible` FROM `aloaz_db`.`user` WHERE `id` = '".$f_uid."'");
	...
	$friend_invisible = $friendUserRow['invisible'];
	
	if(strlen($friend_status) > 50) $friend_status = substr($friend_status, 0, 50).'...';
	
	if($friend_invisible==1){
		$lastOnline = '<span style="font-size:11px; color: green;">Gizli</span>';
	}else{
		if($f_time > (time()-600)) $lastOnline = '<span style="font-size:11px; color: green;">Online</span>'; else $lastOnline = '<span style="font-size:11px;">'.date('Y-m-d H:i',$f_time).'</span>';
	}
	...
	echo '<tr '; echo $i++ % 2 ? ' style="background: #f6f4f4"' : ''; echo '><td width="1%"><a href="profile.php?uid='.$f_uid.'&amp;back=friends"><img src="'.$img_file.'" alt="man" style="border: 1px solid #d7d7d7;width:60px; height:60px;" /></a></td>';
