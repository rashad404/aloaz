...
$checkAuth = checkAuth('`id`, `nickname`, `country_id`');
if($checkAuth != 'error'){
	$userrow = mysql_fetch_array($checkAuth);
	$id = $userrow['id'];
	$country = 'az'; //strtolower($userrow['country_id']);
...
if($_filter == 'friends'){
 ...
 $friendsQuery = mysql_query("SELECT `user_1`, `user_2`, `ok`, `ok_time` FROM `aloaz_db`.`user_friend` WHERE `user_1` = ".$id." OR `user_2` = ".$id." ORDER BY `ok` ASC, `ok_time`");
while($friendsRow = mysql_fetch_array($friendsQuery)){
	...
	$fuid_s = substr($fuid_s, 0, -1);
	$ins_fuid = "`user_id` IN (".$fuid_s.")";
	$ins_filter = $ins_fuid;
...
if($_uid > 0){ 
	$checkFriendQuery = mysql_query("SELECT `id` FROM `aloaz_db`.`user_friend` WHERE (`user_1` = '".$id."' AND `user_2` = '".$_uid."') OR (`user_1` = '".$_uid."' AND `user_2` = '".$id."');");
...
if($_like > 0 and intval($id)>0){
	$shareForLike = mysql_fetch_assoc(mysql_query("SELECT `user_id` FROM `aloaz_db`.`share` WHERE `id`='".$_like."' limit 1;"));
	...
	if(mysql_num_rows($checkLikeQuery) == 0){
		...
		if($shareForLike["user_id"]!=$id){
			setNotification($shareForLike["user_id"],$paramsArray["NOT_SHARE_LIKE"],time(),$id,$nickname,0,$_like);
		}
...
$all_rows_share = mysql_query("SELECT `id` FROM aloaz_db.`share`");
$cnt_all_rows_share = mysql_num_rows($all_rows_share);

if($_filter == 'all') $query = mysql_query("SELECT * FROM (
  SELECT *
  FROM aloaz_db.`share` WHERE status=1 and ".$ins_filter." ".$ins_country."
  ORDER BY time DESC LIMIT ".$cnt_all_rows_share."
) AS share_list GROUP BY user_id ORDER BY `time` DESC LIMIT ".$start.", ".$show_limit.";");

//if($_filter == 'all') $query = mysql_query("SELECT * FROM aloaz_db.`share` GROUP BY `user_id` ORDER BY `time` DESC LIMIT ".$start.", ".$show_limit.";");

else 

$query = mysql_query("SELECT * FROM aloaz_db.`share` WHERE status=1 and ".$ins_filter." ".$ins_country." ORDER BY `time` DESC LIMIT ".$start.", ".$show_limit.";");
//echo $all_rows.'-'.$start.'-'.$show_limit;
...
while($row = mysql_fetch_array($query)){
	...
	if($video_frame!="") $videoImg = ' <img src="/img/play-icon.png" alt="'.$__lng['yalniz dostlarla paylasib'].'" />'; 
	else $videoImg = '';
	...
	echo '<br/><small>'.$date_str.'</small> '.$permissionImg.$videoImg.'<br/>';
	
	if($video_frame!=""){
		if(!empty($attach))
		echo '
			<a class="thickbox play-button-link" href="view.php?id='.$shareId.'" rel="flowplayer" target="" style="z-index: 1; position: relative"> 
				<img border="0" style="padding:5px 0 5px 0;" alt="" src="../udata/images/share/resized/'.date('Ym',$date).'/'.$attach.'" />
				<img class="play-button" alt="" src="/img/play.png" />
			</a><br/>
		';
	}else{
		if(!empty($attach)) echo '<a href="view.php?id='.$shareId.'"><img src="../udata/images/share/resized/'.date('Ym',$date).'/'.$attach.'" alt="." style="padding:5px 0 5px 0;" /></a><br/>';
	}	
	
	echo ''.$text;
	if(!empty($text)) echo '<br/>';
