...
echo '<div class="mnav"><a href="/main.php">AloChat</a> » <a href="/share">'.$__lng['paylash'].'</a></div>';
...
if(!isset($_POST['secnumber'])){
	...
	$_type=htmlspecialchars($_GET["type"]);
	
	if($_type == 'video')
	{ 	
		echo '<a href="add.php">Şekil ve Yazı</a> / ';
		echo  'Video<br/><br/>';
		$videoType=true;
	}
	else{ 
		echo 'Şekil ve Yazı / ';
		echo '<a href="add.php?type=video">Video</a><br/><br/>';
		$videoType = false;
	}
	...
	if($videoType){
		echo 'Video link (Youtube):<br/>';
		echo '<textarea cols="25" rows="2" name="video"></textarea><br/>';
	}else{
	...
	$_video = trim(htmlspecialchars(mysql_escape_string($_POST['video'])));
	$video_status = false;
	$ins_sql = '';
	if($_video!="" and get_youtube($_video)!=NULL){
		
		$getData = get_youtube($_video);
		$video_thumb = $getData["thumbnail_url"];
		$video_title = htmlspecialchars(mysql_escape_string($getData["title"]));
		$video_frame = $getData["html"];
		$video_status = true;
		
		$ins_sql = ",`video_frame`='".$video_frame."',`video_title`='".$video_title."'";
	} 
	...
	*/
	...
	mysql_query("INSERT INTO aloaz_db.`share` SET `status`=1,`user_id` = '".$uid."', `text` = '".$_text."', `permission` = '".$_permission."', `country` = '".$u_country."', `time` = '".time()."'".$ins_sql);
	...
	if(mysql_affected_rows() > 0){
	   ...
	   if($video_status){
			$ext = 'jpg';
			$hash = substr(md5(time().$video_title.substr("abcdefghijklmnopqrstuvwxyz", 0, rand(5, 20))), 0, 15);

			$attach_name = $hash.".".$ext;
			$attach_thumbSmall = 's_'.$hash.".".$ext;
		
			$upload_file = "/home/aloaz/public_html/alochat.com/public_html/images/share/uploads/".$dateDir."/".$attach_name.""; //generate the destination path
			
			$upload_dir = "/home/aloaz/public_html/alochat.com/public_html/images/share/uploads/".$dateDir;
			...
			if($ext != 'jpg' && $ext != 'jpeg' && $ext != 'gif' && $ext != 'png'){
				...
				$result = file_get_contents($video_thumb);
				
				if(file_put_contents($upload_file, $result)){
					...
					$upload_dir = "/home/aloaz/public_html/alochat.com/public_html/images/share/thumbs/".$dateDir;
					if (!is_dir($upload_dir)) {
						mkdir($upload_dir, 0777, true);
					}				
					...
					createthumb2($upload_file, "/home/aloaz/public_html/alochat.com/public_html/images/share/resized/".$dateDir."/".$attach_name."", 220, 250, 0); //medium
				}
			}
		}
	   elseif(!empty($filen)){
