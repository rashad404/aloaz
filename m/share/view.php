...
if($_like > 0 and intval($id)>0){
	...
	if(mysql_num_rows($checkLikeQuery) == 0){
		mysql_query("INSERT INTO aloaz_db.`share_like` SET `uid` = '".$id."', `sid` = '".$_like."', `time` = '".time()."',`from`='alo_view'");
		mysql_query("UPDATE aloaz_db.`share` SET `like_count` = `like_count` + 1 WHERE `id` = '".$_like."'");
		if($row["user_id"]!=$id){
			setNotification($row["user_id"],$paramsArray["NOT_SHARE_LIKE"],time(),$id,$nickname,0,$_like);
		}
...
echo '<small>'.$date_str.'</small> '.$permissionImg.$videoImg.'<br/><br/>';

if($video_frame!=""){
//echo $video_title.'<br/><br/>';	

echo "<div class='video-container'>".$video_frame.'</div><br/><br/>';	
}
elseif(!empty($attach)){
	echo '<a href="../udata/images/share/uploads/'.date('Ym',$date).'/'.$attach.'"><img src="../udata/images/share/resized/'.date('Ym',$date).'/'.$attach.'" alt="." /></a><br/><br/>';
}
