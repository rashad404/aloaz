...
$checkAuth = checkAuth('`id`, `nickname`, `last_post`, `profile_photo`, `sex`, `phone`, `md5_pass`, `coins`, `msg_count`, `no_dating`, `country_id`, `user_status`,`bonus_date`');
if($checkAuth == 'error'){
...
$status = htmlspecialchars(stripslashes($userrow['last_post']));
$sex = $userrow['sex'];
...
$u_bonusday = $userrow['bonus_date'];
 
if($id==1129446){
	if($u_bonusday!=date("Y-m-d")){
		updateBonusDate();
	}
} 
...
if(empty($photo)){
	...
	$profileImg = '<a href="photos/"><img src="'.$img_file.'" alt="man" style="border: 1px solid #d7d7d7" /></a>';
} else{ 	
	$img_file = '/udata'.$photo;
	$profileImg = '<a href="photos/"><img src="'.$img_file.'" alt="man" style="border: 1px solid #d7d7d7;width:60px;height:60px;" /></a>';
}
...
$countNotificationsQuery = mysql_query("SELECT count(`id`) FROM `aloaz_db`.`notification` WHERE `user_id`='".$id."' and `read`=0");
$countNotifications = mysql_result($countNotificationsQuery, 0);

if($newFriendsCount > 0) $newFriendsQueryNot = '+'.$newFriendsCount.'';

if($countNotifications > 0) $countNotificationsTxt = '+'.$countNotifications;
...
if($country == 'az'){
	$query = mysql_query("SELECT * FROM `aloaz_db`.`news` WHERE `time` > '".(time()-3600*24*7)."' ORDER BY `time` DESC LIMIT 3;");
...
if($user_status == 1) {
	$user_status_value = $__lng["user_status_1"].' funksiyaları';
}
elseif($user_status == 2) {
	$user_status_value = $__lng["user_status_2"].' funksiyaları';
}
elseif($user_status == 3){
	$user_status_value = $__lng["user_status_3"].' funksiyaları';
}
elseif($user_status == 10){
	$user_status_value = 'Admin panel';
}
 

if($user_status > 0 and $user_status!=10) echo '* <a href="team-panel.php">'.$user_status_value.'</a><br/><br/>';
elseif($user_status==10) echo '* <a href="admin-panel.php">'.$user_status_value.'</a><br/><br/>';
if($country == 'az') echo '<a target="_blank" href="http://metbuat.az/?ref=aloaz">En son xeberler - METBUAT.AZ</a><br/><br/>';

//if($country == 'az') $online_link = 'online.php'; else $online_link = 'online-global.php';
$online_link = 'online.php';

$roomOnlineCnt = mysql_result(mysql_query("SELECT count(`id`) FROM `aloaz_db`.`user` WHERE `place` > 0 AND `place` < 11 AND `last_activity` > '".(time()-600)."'"), 0);

echo '<div align="center">';
$_ozunutanitnav = intval($_GET['ozunutanitnav']);

//WHERE `country` = '".$country."' 
$ozunutanitQuery = mysql_query("SELECT `uid`, `login`, `status`, `photo_id` FROM `aloaz_db`.`chat_ozunutanit` ORDER BY `id` DESC LIMIT ".abs($_ozunutanitnav).", 1");
...
else $img_file = 'udata'.$ozunutanit_user_photo;
...
echo '<a href="room/index.php"><img src="img/room.png" alt="." /> '.$__lng['sohbet otaqlari'].' ('.$roomOnlineCnt.')</a><br/>';
echo '<a href="messages.php?mod=unread"><img src="img/messages.png" alt="." /> '.$__lng['mesajlar'].'</a> '.$countUnreadNot.'<br/>';
echo '<a href="notifications.php"><img src="img/notifications.png" alt="." width="16px"/> Bildirişler</a> '.$countNotificationsTxt.'<br/>';
...
$new_users = mysql_query("SELECT COUNT(`id`) FROM `aloaz_db`.`user` WHERE `created_at` > '".strtotime(date('Y-m-d 00:00:00'))."'");
$new_users = mysql_result($new_users, 0);

//$cntElan = mysql_result(mysql_query("SELECT COUNT(`id`) FROM `aloaz_db`.`elan`"), 0);
$cntTodayElan = mysql_result(mysql_query("SELECT COUNT(`id`) FROM `aloaz_db`.`elan` WHERE `time` > '".strtotime(date('Y-m-d 00:00:00'))."'"), 0);

if($cntTodayElan > 0) $cntTodayElanTxt = ' (+'.$cntTodayElan.')';
echo '<br/><div align="center">';

echo '<a href="elan.php">Elanlar '.$cntTodayElanTxt.'</a><br/>';
echo '<a href="topactivity.php">Aktivlik reytinqi</a><br/>';
echo '<a href="topposts.php">'.$__lng['top 50 post'].'</a><br/>';
echo '<a href="user-rating.php">İstifadeçi reytinqi ('.monthName().')</a><br/>';
echo '<a href="top-coins.php">Varlı istifadeçiler</a><br/>';
echo '<a href="team.php">Vezifeli istifadeçilər</a><br/>';
echo '<a href="new-users.php">Yeni istifadeçilər ('.$new_users.')</a><br/>';
echo '<a href="stats.php">Statistika</a><br/>';
echo '<br/>';
...
if($country == 'az'){
	...
	echo '<a href="links.php"><img src="img/links.png" alt="." /> '.$__lng['linkler'].'</a><br/><br/>';
	//echo '<a href="support.php"><img src="img/support.png" alt="." /> '.$__lng['bize yaz'].'</a><br/>';
}
echo '<a href="logout.php"><img src="img/logout.png" alt="." /> '.$__lng['cixis'].'</a>';
//echo ' | <a href="http://m.alo.az/alochat.php">Web Versiya</a><br/>';
