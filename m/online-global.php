...
echo '<div class="mnav"><a href="main.php">'.$title.'</a> » '.$__lng['onlayn'].'</div>';
echo '<div class="layer">';
...
$globalOnlineQuery = mysql_query("SELECT `country`, COUNT(`country`) as `cnt_country` FROM `chat_users` WHERE `time` > '".time()."' AND `dating` = 0 GROUP BY `country` LIMIT 100;");
while($globalOnlineRow = mysql_fetch_array($globalOnlineQuery)){
	...
	echo '<a href="online.php?country='.$onlineCountry.'">'.countryCodeName($onlineCountry).' ('.$cnt_country.')</a><br/>';
}
