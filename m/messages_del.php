...
$uidQuery = mysql_query("SELECT `nickname` FROM `aloaz_db`.`user` WHERE `id` = '".$_uid."';");
...
if($_del > 0 && $_uid > 0){ 
	$delete = mysql_query("DELETE FROM `aloaz_db`.`conversation_reply` WHERE `deleted_by`='".$_uid."' and ((`user_id` = '".$id."' AND `user_id_to` = '".$_uid."') OR (`user_id_to` = '".$id."' AND `user_id` = '".$_uid."'));");
	$updateQueryTo = mysql_query("UPDATE `aloaz_db`.`conversation_reply` SET `deleted_by` = '".$id."' WHERE (`user_id` = '".$id."' AND `user_id_to` = '".$_uid."') OR (`user_id_to` = '".$id."' AND `user_id` = '".$_uid."');");
