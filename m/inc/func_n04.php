...
function checkAuthOld($sqlFields = '*'){
	if(isset($_POST['login']) && isset($_POST['password'])){
		$login = checkData($_POST['login']);
	...
	$user_query = mysql_query("SELECT ".$sqlFields." FROM `aloaz_db`.`user` WHERE ".$ins_login." AND `md5_pass` = '".$password."';");
...
function checkOnlyAuth($sqlFields = '*',$safeMode = false)
{
	$login = checkData($_SESSION['login']);
	$password = checkData($_SESSION['password']);
...
function checkAuth($sqlFields = '*',$safeMode = false){
	if(isset($_POST['login']) && isset($_POST['password'])){
		$login = checkData($_POST['login']);
		$login = SqlInjectFilter($login);
		$password = md5(trim($_POST['password']));
	...
	if(mysqli_num_rows($user_query) == 0){
		...
		if(mysqli_num_rows($user_query) == 0){
			...
			if(($user["block_time"]+$user["block_begin_time"])>time() or $user["block_begin_time"]==0){
				...
				$begin_time = $user["block_begin_time"];
				$end_time = $begin_time + $user["block_time"];
				if($begin_time==0){
					$begin_time = time();
					$end_time = $begin_time + $user["block_time"];
					mysql_query("UPDATE `aloaz_db`.`user` SET block_begin_time='".$begin_time."' WHERE id='".$user["id"]."' limit 1");
					mysql_query("UPDATE `aloaz_db`.`blocks` SET `begin_time`='".$begin_time."',`end_time`='".$end_time."' WHERE id='".$block_row["id"]."' order by id desc limit 1");
				}
				...
				$ban_minutes_end = round(($end_time-time())/60);
				
				$block_text = 'Sayta girişiniz <b>'.$from_user_nickname."</b> tərəfindən ".$ban_minutes." dəqiqəlik məhdudlaşdırılıb.<br/><b>".$ban_minutes_end."</b> dəqiqə sonra daxil ola biləcəksiniz.<br/><br/>";
				$block_text .= 'Səbəb: '.$ban_reasons[$block_row['reason']].'<br />';
				if($block_row['reason'] == 4) $block_text .= 'Açıqlama: '.$block_row['description'].'<br />';
				$block_text .= 'Daxil ola biləcəyiniz tarix: '.date('d-m-Y H:i',$end_time).'<br />';
				if($block_row['time']<=3600){
					$block_text .= '<br /><a href="/remove-ban.php">Ban leğv et</a>';
				}
				
				if($safeMode==false){
					//session_destroy();
					displayError($block_text,2);
					echo '<div class="graynav"><a href="/index.php">m.alo.az</a></div><div class="layer">© 2016 AloChat</div></body></html>';
					exit();
				}
...
function getReasons()
{
	$reasons = array(1=>"Reklam",2=>"Təhqir",3=>"Spam",4=>"Digər");
	return $reasons;
}
...
function updateOnlineOld(){
	...
	//$update = mysql_query("UPDATE `chat_users` SET `last_activity` = '".$online."',`last_activity_round` = '".$online_round."' ,`place` = '".$_rid."', `onfrom` = 'mobile', `ip` = '".getenv('REMOTE_ADDR')."', `ua` = '".htmlspecialchars(getenv('HTTP_USER_AGENT'))."' WHERE `id` = '".$id."' LIMIT 1;");
	$update = mysql_query("UPDATE `aloaz_db`.`user` SET `last_activity` = '".$online."',`last_activity_round` = '".$online_round."' ,`place` = '".$_rid."', `onfrom` = 'mobile', `ip` = '".getenv('REMOTE_ADDR')."', `ua` = '".htmlspecialchars(getenv('HTTP_USER_AGENT'))."' WHERE `id` = '".$id."' LIMIT 1;");
...
function updateOnline()
{
	...
	$_rid = intval($_rid);
	$time = time();
	$checkAuth = checkAuth('`daily_activity`, `weekly_activity`, `monthly_activity`, `last_activity`');
	$userrow = mysql_fetch_array($checkAuth);

	$daily_activity = $userrow["daily_activity"];
	$weekly_activity = $userrow["weekly_activity"];
	$monthly_activity = $userrow["monthly_activity"];
	if(($time - $userrow["last_activity"])<600){
		$add_activity = $time - $userrow["last_activity"];
		$daily_activity = $userrow["daily_activity"] + $add_activity;
		$weekly_activity = $userrow["weekly_activity"] + $add_activity;
		$monthly_activity = $userrow["monthly_activity"] + $add_activity;
	}
	$online = $time;// + 600
	$online_round = substr($time,0,7);
	 
	$update = mysql_query("UPDATE `aloaz_db`.`user` SET `daily_activity`='".$daily_activity."',`weekly_activity`='".$weekly_activity."',`monthly_activity`='".$monthly_activity."',`last_activity` = '".$online."',`last_activity_round` = '".$online_round."' ,`place` = '".$_rid."', `onfrom` = 'mobile', `ip` = '".getenv('REMOTE_ADDR')."', `ua` = '".htmlspecialchars(getenv('HTTP_USER_AGENT'))."' WHERE `id` = '".$id."' LIMIT 1;");
...
function getPortmanat()
{
	$array = array(
		'portmanat_partner_id' => '10575',
		'portmanat_service_id' => '11943',
		'portmanat_pass' => 'a5V15ePld',
	);

	return $array;
}

function setNotification($user_id,$type,$time=0,$user_id_from=1,$username='AloChat',$coin=0,$share_id=0,$read=0,$image='')
{ 
	if($image == ''){ 
		$user = mysql_fetch_assoc(mysql_query("SELECT `profile_photo` FROM `aloaz_db`.`user` WHERE id='".$user_id_from."'"));
		if($user){
			$image = $user['profile_photo']?$user['profile_photo'] : '/images/icons/sample.png';
		}else{
			$image = '/images/icons/sample.png';
		}
	}
	$insert = mysql_query("INSERT INTO `aloaz_db`.`notification` SET user_id='".$user_id."',user_id_from='".$user_id_from."',coin='".$coin."',`type`='".$type."',username='".$username."',share_id='".$share_id."',`read`='".$read."',`time`='".$time."',`image`='".$image."'");
...
function updateBonusDate()
{
	...
	$time = time();
	$checkAuth = checkAuth('`bonus_date`, `bonus_day_count`');
	$userrow = mysql_fetch_array($checkAuth);

	$bonus_date = $userrow["bonus_date"];
	$bonus_day_count = $userrow["bonus_day_count"]; 
	
	if($bonus_day_count<=7){
		if($bonus_date==date("Y-m-d", strtotime( '-1 days' ) )){
			$new_bonus_day_count = $bonus_day_count + 1;
		}else{
			$new_bonus_day_count = 1;
		}
		
		$update = mysql_query("UPDATE `aloaz_db`.`user` SET `bonus_date`='".date("Y-m-d")."',`bonus_day_count`='".$new_bonus_day_count."' WHERE `id` = '".$id."' LIMIT 1;");
		if($update) return true; else return false;
	}	
}

function getNickFonts()
{
	$fontArray = array('tahoma', 'arial', 'comic-sans-ms', 'bahamabd', 'austere-ssi', 'a-simpler-3d');
	return $fontArray;
}

function getNickColors(){
	$colorArray = array("yellow" => "Sarı","green" => "Yaşıl","red" => "Qırmızı","blue" => "Mavi","black"=>"Qara","gray"=>"Boz","white"=>"Ağ","brown"=>"Qehveyi","purple"=>"Bənövşəyi","pink"=>"Çəhrayı");
	return $colorArray;
}

function timeToDate($difference)
{ 
	$array["d"] = $days = floor($difference / 86400);
	$array["H"] = $hours = floor(($difference - $days * 86400) / 3600);
	$array["i"] = $minutes = floor(($difference - $days * 86400 - $hours * 3600) / 60);
	$array["s"] = $seconds = floor($difference - $days * 86400 - $hours * 3600 - $minutes * 60);
	return $array;		
}
