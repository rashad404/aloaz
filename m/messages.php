...
$admin_status = 0;
$checkAuth = checkAuth('`id`, `nickname`, `phone`, `coins`, `only_friend`, `post_run`, `msg_count_day`, `mysmile`, `user_status`');
if($checkAuth == 'error'){
...
if(strlen($phone) != 12){
	$count_conversation = mysql_query("SELECT `id` FROM aloaz_db.`conversation` WHERE `user_one` = '".$id."' AND `last_time` > '".strtotime(date('Y-m-d 00:00'))."';");
	if(mysql_num_rows($count_conversation) > 20){
		echo "<b>Günlük mesaj yazmaq limitini keçmisiniz.</b> Limitsiz mesajlaşmaq üçün nömrənizi təsdiqləyin. Nömrə təsdiqləmək pulsuzdur. - <a href='submitphone.php'>Təsdiqlə</a><br/>\n";
		echo '<br/><a href="javascript:history.back(1)">« '.$__lng['geri'].'</a><br/>';
		break;
	}
}
...
if($uid_friends == 1 && mysql_num_rows($checkFriendQuery) == 0 && $admin_status == 0){ 
	echo "<b>".$__lng['dostlardan mesaj qebul edir']."..</b><br/>"; 
	echo '<br/><a href="javascript:history.back(1)">« '.$__lng['geri'].'</a><br/>';
	break;
}

if($uid_friends == 1 && ($ok1 != 1 || $ok2 != 1)  && $admin_status == 0){
...
$checkLimit = mysql_result(mysql_query("SELECT COUNT(`id`) FROM aloaz_db.`conversation_reply` WHERE `user_id` = '".$id."' AND `time` > '".(time()-60)."'"), 0);
if($checkLimit > 20){
	$error = true;
...
if($count_unread == 0){
	...
	echo $count_unread." ".$__lng['oxunmamis mesajiniz var'].'<br/><br/>';
