...
echo '<div class="mnav"><a href="/main.php">AloChat</a> » <a href="news.php?mod=read&amp;id='.$_id.'">'.$__lng['sherhler'].'</a></div>';
echo '<div class="layer">';

$checkQuery = mysql_query("SELECT `id` FROM aloaz_db.`news` WHERE `id` = '".$_id."';");
...
if(!isset($_POST['secnumber'])){
	...
	$checkLimit = mysql_result(mysql_query("SELECT COUNT(`id`) FROM aloaz_db.`news_comment` WHERE `user_id` = '".$uid."' AND `news_id` = '".$_id."' AND `time` > '".(time()-60)."'"), 0);
	if($checkLimit > 5){
		$error .= '- Spam!!!<br/>';
	...
	mysql_query("INSERT INTO aloaz_db.`news_comment` SET `user_id` = '".$uid."', `news_id` = '".$_id."', `comment` = '".$comment."', `time` = '".time()."'");
