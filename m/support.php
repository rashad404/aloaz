...
case 'add':
	if($_POST['submit'] == ''){
		...
		$checkQuery = mysql_query("SELECT `id` FROM `aloaz_db`.`chat_support` WHERE `uid` = '".$id."' AND `answer` = '';");
		...
		mysql_query("INSERT INTO `aloaz_db`.`chat_support` SET `uid` = '".$id."', `type` = '".$_type."', `title` = '".$_title."', `body` = '".$_body."', `time` = '".time()."'");
...
$query = mysql_query("SELECT * FROM `aloaz_db`.`chat_support` WHERE `uid` = '".$id."' ORDER BY `time` DESC LIMIT 10;");
