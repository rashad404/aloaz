...
if(!isset($_POST['submit'])){
	...
	$query = mysql_query("SELECT `password`, `phone` FROM `aloaz_db`.`user` WHERE ".$sql_login.";");
	...
	if(mysql_num_rows($query) == 0){
		...
		if(strlen($user_phone) != 12){
			...
			if($_POST['complate'] != 'ok'){
				$checkQuery = mysql_query("SELECT `id` FROM `aloaz_db`.`chat_pincodes` WHERE `phone` = '".$user_phone."' AND `used` = '0' AND `time` > '".(time()-3600*12)."'");
				if(mysql_num_rows($checkQuery) == 0){
					$pincode = rand(1111,9999);
					mysql_query("INSERT INTO `aloaz_db`.`chat_pincodes` SET `phone` = '".$user_phone."', `pin` = '".$pincode."', `ip` = '".$ip."', `ua` = '".$ua."', `time` = '".time()."'");
				...
				if(strlen($_newpass) < 6){
					...
					$checkPinQuery = mysql_query("SELECT `id` FROM `aloaz_db`.`chat_pincodes` WHERE `phone` = '".$user_phone."' AND `pin` = '".$_pincode."' AND `used` = '0' AND `time` > '".(time()-3600*12)."'");
					if(mysql_num_rows($checkPinQuery) == 0){
						echo $__lng['tesdiq kodu yanlisdir'].'.<br/>';
					}
					else{
						mysql_query("UPDATE `aloaz_db`.`user` SET `password` = '".$_newpass."', `md5_pass` = '".md5($_newpass)."',`changed_pass`=1 WHERE `phone` = '".$user_phone."' LIMIT 1");
						if(mysql_affected_rows() > 0){
							echo $__lng['sifre deyisdirildi'].'<br/>';
							$user = mysql_fetch_assoc(mysql_query("SELECT `id` FROM `aloaz_db`.`user` WHERE `phone` = '".$user_phone."' and `verify`=1 limit 1"));
							if($user){
								$url  = 'http://alochat.com/alo/change-pass/'.$user["id"];	 		
						...
						mysql_query("UPDATE `aloaz_db`.`chat_pincodes` SET `used` = '1' WHERE `phone` = '".$user_phone."' AND `pin` = '".$_pincode."'");
					}
				}
			}
		}
