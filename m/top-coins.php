...
if($_period == 'month'){ 
	$where_period = " and `t`.`update_date`>'".date("Y-m-d H:i:s",strtotime(date("Y-m")."-01"))."'";
...
$query = mysql_query("SELECT sum(`t`.`coins`) as s,t.id,t.user_id,u.nickname,`u`.`sex`,`u`.`profile_photo`,`u`.`last_post`,`u`.`coins`,`u`.`age` FROM `aloaz_db`.`transactions` as `t` RIGHT OUTER JOIN `aloaz_db`.`user` as `u` ON `t`.`user_id`=`u`.`id`
 WHERE `payment_status`=1 ".$where_period." GROUP BY `t`.`user_id` ORDER BY s DESC LIMIT 20");
$num = $start;  
$i=1;	 	
